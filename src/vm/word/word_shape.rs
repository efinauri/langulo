...
pub const IS_OPTION_FLAG_MASK: u64 = bitmask(IS_OPTION_FLAG_START as u32, IS_NONE_FLAG_START as u32);
...
impl Debug for Word {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        {:032b} {:021b} {:06b} {:01b} {:01b} {:03b}",
               ...
               "ptr", "chaff", "opcode",
               "B", "A", "tag",
               self.ptr() as u32, self.chaff(), self.opcode().to_u8().unwrap(),
               self.is_option() as u8, self.is_none() as u8, self.tag().to_u8().unwrap()
        )
    }
}

impl Word {
    pub fn ptr(self) -> *mut u8 { ((self.0 as u64 & PTR_MASK) >> PTR_START) as _ }
    pub fn chaff(&self) -> u32 { ((self.0 as u64 & CHAFF_MASK) >> CHAFF_START) as _ }
    pub fn is_option(&self) -> bool { ((self.0 as u64 & IS_OPTION_FLAG_MASK) >> IS_OPTION_FLAG_START) == 1 }
    pub fn is_none(&self) -> bool { ((self.0 as u64 & IS_NONE_FLAG_MASK) >> IS_NONE_FLAG_START) == 1 }
    pub fn opcode(&self) -> OpCode { OpCode::from_u64((self.0 as u64 & OPCODE_FLAG_MASK) >> OPCODE_START).unwrap() }
    pub fn tag(&self) -> ValueTag { ValueTag::from_u64(self.0 as u64 & TAG_MASK).unwrap() }
...
mod tests {
    ...
    fn new_word() {
        ...
        let word = Word::new(0x69420 as _, true, true, OpCode::JumpIfFalse, ValueTag::StrPtr);
        println!("{:?}", word);
